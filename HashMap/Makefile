...
test_hashmap_iter: hashmap_tests
	$(ENV_VARS) ./$< --gtest_color=yes --gtest_filter="HashMapIterating*"
...
markov_main: markov.cpp hashmap.h
	$(CXX) $(CXXFLAGS) markov.cpp -lgtest -lgmock -lgtest_main -o $@

run_markov: markov_main
	$(ENV_VARS) ./$<

clean:
	rm -f hashmap_tests hashmap_main markov_main build/*
	# MacOS symbol cleanup
	rm -rf *.dSYM

.PHONY: clean run_main run_markov test_hashmap_core test_hashmap_iter test_hashmap_all